...
- All global variables that are declared as `const` are stored in read-only flash memory, right next to code. In most Nano Apps for example, general settings are stored in flash memory.
